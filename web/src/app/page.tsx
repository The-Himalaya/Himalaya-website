...
function FirstSlideText({ slide }: { slide: Slide }) {
  const fullHeading = slide.heading + ' ' + slide.headingAccent;
  const { displayed: typedHeading, done: headingDone } = useTyping(fullHeading, 55, 400);
  const { displayed: typedSub } = useTyping(slide.sub, 20, 0, headingDone);

  const headingPart = typedHeading.slice(0, slide.heading.length);
  const accentPart = typedHeading.slice(slide.heading.length + 1);

  return (
    <>
      <h1 className="text-5xl md:text-7xl lg:text-8xl font-display uppercase leading-none text-white mb-4">
        {headingPart}
        {typedHeading.length > slide.heading.length && <br />}
        <span style={{ color: slide.accent }}>
          {accentPart}
          {!headingDone && <span className="animate-pulse">|</span>}
        </span>
      </h1>
      <p className="text-lg md:text-xl text-white/60 max-w-2xl mt-6 leading-relaxed min-h-[2em]">
        {typedSub}
        {headingDone && typedSub.length < slide.sub.length && (
...
function HeroSlide({
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ y, scale }} className="relative z-10 w-full px-8 md:px-16 lg:px-24 max-w-4xl">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] mb-6" style={{ color: slide.accent }}>
          {slide.label}
        </p>

        {index === 0 ? (
          <FirstSlideText slide={slide} />
        ) : (
          <>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-display uppercase leading-none text-white mb-4">
              {slide.heading}
        ...
        {slide.cta && (
          <div className="flex flex-col sm:flex-row gap-4 mt-10">
            ...
            >
              <Link href="/bulk-order">Request Bulk Quote</Link>
            </Button>
          </div>
        )}
      </motion.div>
...
export default function Home() {
  ...
  const slides: Slide[] = [
    {
      label: `${yearsActive}+ Years of Excellence`,
      heading: 'Built to Last.',
      headingAccent: 'Built for India.',
      sub: `${s.tagline} — Manufacturing world-class infrastructure products since ${s.established_year}.`,
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(project.products ?? project.productsUsed ?? []).map((product, i) => (
                      <span
                        key={i}
